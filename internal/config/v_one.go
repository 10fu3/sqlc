...
type v1PackageSettings struct {
	...
	EmitEmbedNullableLeftJoin bool              `json:"emit_embed_nullable_left_join,omitempty" yaml:"emit_embed_nullable_left_join"`
		...
		conf.SQL = append(conf.SQL, SQL{
			...
			Gen: SQLGen{
				Go: &golang.Options{
					...
					EmitEmbedNullableLeftJoin: pkg.EmitEmbedNullableLeftJoin,
					Package:                   pkg.Name,
					Out:                       pkg.Path,
